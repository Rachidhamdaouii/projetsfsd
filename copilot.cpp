...
void defragmenter_fichier(Fichier *fichier) {
    int index_libre = 0;
    for (int i = 0; i < fichier->nb_enregistrements; i++) {
        if (fichier->enregistrements[i].id != -1) {  // Supposons que les enregistrements supprimés ont l'ID -1
            if (i != index_libre) {
    ...
    // Réduire le nombre d'enregistrements pour ne compter que les enregistrements non supprimés
    fichier->nb_enregistrements = index_libre;

    printf("Défragmentation du fichier %s terminée.\n", fichier->nom);
}
...
int main() {
    ...
    while (1) {
        ...
        switch (choix) {
            ...
            case 7:
                printf("Nom du fichier à défragmenter : ");
                scanf("%s", nom);
                // Recherche du fichier par son nom
                // On suppose ici que la fonction `trouver_fichier` retourne un pointeur sur le fichier trouvé.
